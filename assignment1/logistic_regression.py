...
def logistic_regression(X: ad.Node, W: ad.Node, b: ad.Node) -> ad.Node:
    ...
    """
    logits = ad.matmul(X,W)
    b_broadcasted = ad.broadcast_to_like(b,logits)
    return logits + b_broadcasted
...
def softmax_loss(Z: ad.Node, y_one_hot: ad.Node, batch_size: int) -> ad.Node:
    ...
    """
    log_sum_exp = ad.log(ad.sum(ad.exp(Z), axis=1, keepdims=True))  # shape: (batch_size, 1)
    correct_logit = ad.sum(Z * y_one_hot, axis=1, keepdims=True)    # shape: (batch_size, 1)
    loss_vector = log_sum_exp - correct_logit                       # shape: (batch_size, 1)
    loss = ad.sum(loss_vector) / batch_size
    return loss
...
def sgd_epoch(
    ...
    for start in range(0, num_examples, batch_size):
        ...
        # print("y_batch_one_hot=",y_batch_one_hot.shape)
        logits, loss, dW, db = f_run_model(X_batch, y_batch_one_hot, W, b)
